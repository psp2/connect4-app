...
def play_game(board_size: int, num_players: int, silly_mode: bool) -> int:
    if num_players == 1:
        while True:
            try:
    ...
    while True:
        if cur_player == 2 and num_players == 1:
            ...
            print_blue('AI\n', '')
            if silly_mode:
                selected_col = choose_worst_column(board, board_size, difficulty)
            else:
...
def game_setup():
    ...
    return num_players, board_size


def run_game():
    signal.signal(signal.SIGINT, sigint_handler)
    num_players, board_size = game_setup()
    # Play Game with 1 or 2 players
    winner = play_game(board_size, num_players, False)
    # Results
    if winner == 1:
...
def run_silly_game():
    signal.signal(signal.SIGINT, sigint_handler)
    print('Silly Connect 4')
    print('Whoever connects 4 first loses!\n')
    num_players, board_size = game_setup()
    # Play Game with 1 or 2 players
    winner = play_game(board_size, num_players, True)
    # Results
    if winner == 1 and num_players == 2:
        print_blue('Player 2 has Won.\n', '')
    elif winner == 1:
        print_blue('The AI has Won.\n', '')
    elif winner == 2:
        ...
        print('Somehow, the game has ended in a Tie.\n')
...
if __name__ == '__main__':
    while True:
        ...
        while True:
            try:
                play_again = input('Play Again? Type Y for Yes, N for No, or S for Silly Connect 4! ')
            except ValueError:
                print('Invalid Input.')
                continue
            if play_again == 'Y' or play_again == 'y':
                run_game()
            elif play_again == 'S' or play_again == 's':
                run_silly_game()
